...
Automated installer for Xray VLESS with WebSocket+TLS, XHTTP, Reality, Cloudflare WARP, CDN, Relay, Psiphon, and Tor support.
...
- ✅ **VLESS + XHTTP** — HTTP/2 CDN-compatible transport over nginx on port 443
...
Client (XHTTP — CDN)
    └── CDN/domain:443/HTTPS → nginx → VLESS+XHTTP → Xray → outbound
...
|------|---------|
| 22 | SSH (configurable) |
| 443 | VLESS+WS+TLS and XHTTP via Nginx |
...
| 45000–45999¹ | VLESS+XHTTP (internal, auto-assigned free port) |
...
The `.txt` file is base64-encoded and contains all connection links (WS+TLS, Reality, and XHTTP if installed).
...
CDN-compatible HTTP/2 transport. Runs as a separate `xray-xhttp` service listening locally; nginx proxies the configured path to it on port 443.

**How it works:**

```
Client → CDN/domain:443/HTTPS → nginx (http2 on) → 127.0.0.1:LPORT (Xray XHTTP)
```

- Works with or without Cloudflare CDN — just toggle the orange cloud
- Local port is auto-assigned from the free range 45000–45999
- Uses its own UUID independent of WS
- All routing features (WARP, Relay, Psiphon, Tor, Adblock, Privacy) apply automatically
- Requires nginx ≥ 1.25.1 (for `http2 on` with ALPN negotiation)

**Connection link format:**
```
vless://UUID@domain:443?security=tls&type=xhttp&path=/api/v2/xxxx&sni=domain&fp=chrome&allowInsecure=0
```
...
| Xray `xhttp.json` | `access: none`, `loglevel: none` |
...
Both methods are available for WS SSL (item 3 → 5).
...
| XHTTP | Config validity, xray-xhttp service, nginx location |
...
Includes: Xray configs (WS, Reality, XHTTP), Nginx + SSL certs, Cloudflare API key, cron tasks, Fail2Ban rules, xray-xhttp systemd service, sysctl settings.
...
├── xhttp.sh      # VLESS+XHTTP (CDN transport)
...
├── diag.sh       # Diagnostics (incl. XHTTP)
...
├── xhttp.json               # VLESS+XHTTP config
├── vwn.conf                 # VWN settings (lang, domain, XHTTP_UUID, XHTTP_PATH, XHTTP_LPORT…)
...
│   ├── cpuguard.conf
│   └── no-journal.conf
├── xray-xhttp.service         # XHTTP systemd unit
│   └── no-journal.conf
├── nginx.service.d/
│   └── cpuguard.conf
├── user.slice.d/
...
Client → domain:443 → nginx → Xray XHTTP → outbound
```
...
| **Timeout XHTTP, logs empty** | Before Xray XHTTP | `ss -tlnp \| grep 450` | Port not listening → `systemctl restart xray-xhttp` |
...
Автоматический установщик Xray VLESS с поддержкой WebSocket+TLS, XHTTP, Reality, Cloudflare WARP, CDN, Relay, Psiphon и Tor.
...
ision использует raw TLS, Cloudflare не может проксировать такой трафик.

## Требования

- Ubuntu 22.04+ / Debian 11+
- Доступ root
- Домен, указывающий на сервер (для WS+TLS)
...
- ✅ **VLESS + XHTTP** — HTTP/2 CDN-совместимый транспорт через nginx на порту 443
...
- ✅ **Бэкап и восстановление** — ручной бэкап/восстановление/удаление всех конфигов
- ✅ **Диагностика** — полная проверка с разбивкой по компонентам
...
Клиент (XHTTP — CDN)
    └── CDN/домен:443/HTTPS → nginx → VLESS+XHTTP → Xray → outbound
...
| 443 | VLESS+WS+TLS и XHTTP через Nginx |
...
| 45000–45999¹ | VLESS+XHTTP (внутренний, автовыбор свободного порта) |
...
----------------------------------------------------------------
  ...
  5.  Управление XHTTP (CDN транспорт)
...
- Изменения применяются мгновенно к конфигам WS, Reality и XHTTP
...
Файл `.txt` закодирован в base64 и содержит все ссылки подключения (WS+TLS, Reality и XHTTP если установлен).
...
CDN-совместимый HTTP/2 транспорт. Работает как отдельный сервис `xray-xhttp`, слушает локально; nginx проксирует настроенный путь на него через порт 443.

**Как работает:**

```
Клиент → CDN/домен:443/HTTPS → nginx (http2 on) → 127.0.0.1:LPORT (Xray XHTTP)
```

- Работает с CDN и без — достаточно переключить оранжевое облако Cloudflare
- Локальный порт автовыбирается из диапазона 45000–45999
- Использует собственный UUID, независимый от WS
- Все функции маршрутизации (WARP, Relay, Psiphon, Tor, Adblock, Privacy) применяются автоматически
- Требует nginx ≥ 1.25.1 (для `http2 on` с ALPN)
...
Использует `geosite:category-ads-all` — встроенную категорию в `geosite.dat` Xray, обновляемую вместе с Xray. Применяется к конфигам WS, Reality и XHTTP одновременно.
...
Все туннели поддерживают режимы **Global / Split / OFF**. Применяются к конфигам WS, Reality и XHTTP одновременно.
...
**Global** — весь трафик через WARP. **OFF** — отключён от роутинга. Применяется к конфигам WS, Reality и XHTTP.
...
Оба метода доступны для WS (пункт 3 → 5).
...
| XHTTP | Конфиг, xray-xhttp, nginx location |
...
Включает: конфиги Xray (WS, Reality, XHTTP), Nginx + SSL, API-ключи Cloudflare, cron, Fail2Ban, systemd-юнит xray-xhttp, настройки sysctl.
...
├── reality.sh    # VLESS+Reality
├── xhttp.sh      # VLESS+XHTTP (CDN транспорт)
...
├── diag.sh       # Диагностика (включая XHTTP)
...
├── xhttp.json               # Конфиг VLESS+XHTTP
├── vwn.conf                 # Настройки VWN (язык, домен, XHTTP_UUID, XHTTP_PATH, XHTTP_LPORT…)
...
│   ├── cpuguard.conf
│   └── no-journal.conf
├── xray-xhttp.service         # Systemd-юнит XHTTP
│   └── no-journal.conf
├── nginx.service.d/
│   └── cpuguard.conf
├── user.slice.d/
...
Клиент → domain:443 → nginx → Xray XHTTP → outbound
```
...
| **Timeout XHTTP, логи пустые** | До Xray XHTTP | `ss -tlnp \| grep 450` | Порт не слушается → `systemctl restart xray-xhttp` |

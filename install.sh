...
CHECKSUMS_URL="https://raw.githubusercontent.com/HnDK0/VLESS-WebSocket-TLS-Nginx-WARP/main/checksums.sha256"
...
download_modules() {
    ...
    # Скачиваем checksums если доступны
    local checksums_file=$(mktemp)
    local checksums_available=false
    if curl -fsSL --connect-timeout 15 "${CHECKSUMS_URL}" -o "$checksums_file" 2>/dev/null; then
    ...
    for module in $MODULES; do
        ...
        if curl -fsSL --connect-timeout 15 \
            ...
            # Проверяем контрольную сумму если доступна
            if $checksums_available; then
                local expected_hash=$(grep "modules/${module}.sh" "$checksums_file" 2>/dev/null | awk '{print $1}')
                if [ -n "$expected_hash" ]; then
                    local actual_hash=$(sha256sum "$tmpfile" 2>/dev/null | awk '{print $1}')
                    if [ "$expected_hash" != "$actual_hash" ]; then
                        echo "${red}$(msg error) (checksum mismatch)${reset}"
